...
export function useLocationSelector(
  ...
  // Helper to get display name from GeoUnit (handles multiple field formats)
  const getUnitLabel = (unit: GeoUnit, fallback: string): string => {
    return unit.display_name || unit.name || unit.name_en || unit.name_ne || fallback;
  };
    ...
    () =>
      (provincesData || [])
        ...
        .map((unit) => ({
          ...
          label: getUnitLabel(unit, `Province ${unit.id}`),
        })),
    [provincesData]
  );
  ...
  const districtOptions = useMemo(
    () =>
      (districtsData || [])
        ...
        .map((unit) => ({
          value: String(unit.id),
          label: getUnitLabel(unit, `District ${unit.id}`),
        })),
    [districtsData]
  );
  ...
  const localBodyOptions = useMemo(
    () =>
      (localBodiesData || [])
        ...
        .map((unit) => ({
          value: String(unit.id),
          label: getUnitLabel(unit, `Local Body ${unit.id}`),
        })),
    [localBodiesData]
  );
    ...
    () =>
      (wardsData || [])
        ...
        .map((unit) => ({
          ...
          label: unit.ward_no
            ? `Ward ${unit.ward_no}`
            : getUnitLabel(unit, `Ward ${unit.id}`),
        })),
    [wardsData]
  );
  ...
  const getFullLocationPath = useCallback(() => {
    ...
    if (selectedProvince) {
      ...
      if (province) parts.push(getUnitLabel(province, "Province"));
    }
    if (selectedDistrict) {
      const district = districtsData?.find(
        (d) => String(d.id) === selectedDistrict
      );
      if (district) parts.push(getUnitLabel(district, "District"));
    }
    if (selectedLocalBody) {
      const localBody = localBodiesData?.find(
        (l) => String(l.id) === selectedLocalBody
      );
      if (localBody) parts.push(getUnitLabel(localBody, "Local Body"));
    }
    if (selectedWard) {
      const ward = wardsData?.find((w) => String(w.id) === selectedWard);
      if (ward) {
        parts.push(ward.ward_no ? `Ward ${ward.ward_no}` : getUnitLabel(ward, "Ward"));
      }
    }
    return parts.join(" > ");
  }, [
